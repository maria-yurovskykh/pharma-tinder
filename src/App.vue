    ...
    <transition name="fade">
      <div v-if="step === 1" class="home" key="1">
        <div class="home__circle">
          <p class="home__welcome">Ласкаво просимо y гру</p>
      ...
      <div v-else-if="step === 2" class="game" key="2">
        <div class="game__side-bar">
          ...
          <div class="side-bar__counter">
            ...
            <p class="counter__left">
              {{ queueCount }}/{{ clientsDataLength }}
            </p>
          </div>
        </div>
        ...
        <div class="game__screen">
          ...
          <button class="screen__button --purple" @click="throwleft()">
            Препарат 1
          </button>
          <button class="screen__button --blue" @click="throwup()">
            Препарат 2
          </button>
          <button class="screen__button --yellow" @click="throwright()">
            Препарат 3
          </button>
        </div>
      </div>
      ...
      <div v-else-if="step === 3" class="final" key="3">
        ...
        <div class="final__counter">
          <div class="counter__percents --drug-1">
            {{ countPercentOfSales(this.sad) }}%
          </div>
          <div class="counter__percents --drug-2">
            {{ countPercentOfSales(this.happy) }}%
          </div>
          <div class="counter__percents --drug-3">
            {{ countPercentOfSales(this.heart) }}%
          </div>
        ...
        <div class="final__result">
          ...
          <p class="result__content">
            Це тестове завдання, так що не будемо заглиблюватися в глибини
            проблем фармацевтів.
          </p>
        </div>
        <button class="final__restart" @click="setStep(2)">
          Спробувати ще
        </button>
      </div>
    </transition>
